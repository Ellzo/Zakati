...
public class ZakatMonnaiePanel extends ScreenPanel implements ActionListener{
	private Projet p;
	...
	public ZakatMonnaiePanel(Projet p) {
		this.p = p;
		
		titleLabel.setText(Consts.ZAKAT_MONNAIE);
		
		JPanel montantPanel = new JPanel();
		montantPanel.setLayout(new FlowLayout());
		montantPanel.add(new JLabel(Consts.ENTRER_MONTANT));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == calcBtn) {
			montantZakat = (int) Math.round(Calculations.calcZakatMonnaie(Integer.parseInt(montantField.getText())));
